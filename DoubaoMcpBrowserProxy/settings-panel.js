...
(function() {
    ...
    ">
      ...
      ">
        ...
        <div class="setting-item" style="
          ...
          transition: all 0.2s ease;
        ">
          <div style="font-weight: 500; color: #2c3e50; margin-bottom: 4px;">Cookie 保持</div>
          <div style="color: #7f8c8d; font-size: 11px; line-height: 1.3;">
            当前不会在任务完成后自动清除豆包 Cookie。
          </div>
        </div>
        ...
        ">
          ...
          " placeholder="留空时自动探测">
          <div style="color: #7f8c8d; font-size: 11px; margin-top: 4px; line-height: 1.3;">
            可手动指定 WebSocket 地址；留空时会依次尝试 ws://browser-api:8080、ws://host.docker.internal:8080、ws://localhost:8080。
          </div>
        </div>
  ...
  async function loadSettingsPanel() {
    ...
    async function loadOptions() {
      try {
        const result = await storageGet(['autoReload', 'wsUrl']);
        document.getElementById('autoReload').checked = result.autoReload !== undefined ? result.autoReload : false;
        document.getElementById('wsUrl').value = result.wsUrl || '';
      } catch (error) {
